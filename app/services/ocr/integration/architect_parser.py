...
class ArchitectVisualParser:
    ...
    async def _ocr_consumer(self, raw_texts: Dict[int, str]):
        ...
        while True:
            ...
            async with self.semaphore:
                try:
                    ...
                    md_content = await worker.ocr_to_markdown(img_np)
                    raw_texts[page_idx] = md_content
                    print(f"📄 [TOC-Sense] 页面 P{page_idx+1} 识别完成")
                    
                    # 🚀 [V1.2.8] 云端强制冷却，防止 429
                    if "Zhipu" in str(type(worker)):
                        import random
