...
async def _finalize_ingestion(
    ...
    if not skip_bitable:
        try:
            ...
            # 5. Git 溯源 (恢复逻辑，拒绝简化)
            print(f"📜 [Finalize] 提交 Git 逻辑指纹...")
            for c in chunks:
                git_manager.commit_chunk(
                    chunk_id=str(uuid4()), 
                    content=c.get("content", ""),
                    message=f"Ingestion Sync: {db_doc.filename}"
                )

        except Exception as e:
            print(f"⚠️ [Finalize] 确权崩溃: {e}")
            import traceback
            traceback.print_exc()
    else:
        print(f"🛰️ [Finalize] 旁路确权开启。")
