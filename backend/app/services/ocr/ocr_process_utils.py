...
class AdaptiveOCRWorker:
    ...
    def _initialize(self):
        ...
        try:
            ...
            print(" [OCR] PaddleWorker 已就绪（延迟加载）")
        except Exception as e:
            print(f" [OCR] PaddleWorker 加载失败: {e}")

        # 2. GOTWorker 延迟加载，不在这里 raise
        try:
            self.got_worker = GOTWorker()
            print(" [OCR] GOTWorker 已就绪")
        except Exception as e:
            print(f" [OCR] GOTWorker 加载失败，将使用云端降级: {e}")
        ...
        if vlm_key:
            ...
            print(" [OCR] 高精度 VLM (SiliconFlow) 已就绪")
    ...
    async def ocr_to_markdown(self, img_np: np.ndarray, high_precision: bool = False) -> Optional[str]:
        ...
        # 2.  [V1.8.0] 确定性路径：Paddle (Scout) + GOT (Specialist)
...
#  全局单例缓存，防止重复加载模型导致 OOM (Operation: Unified Brain)
